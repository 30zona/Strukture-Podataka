...
int main() {
    pozLista head[11];
    for (int i = 0; i < 11; i++)
        head[i] = stvaranjeLista();
    for (int i = 0; i < 11; i++)
        ispisSvihDrzava(head[i]);
    userInterface(head);
    clearLista(head);
    return 0;
}
...
int sortiraniUnosIzDatoteke(char* filename, pozLista head[])
{
    ...
    while (!feof(fp))
    {
        ...
        sortiraniUnos(l, head[kodiranje(l->drzava)]); //unosimo drzavu, zajedno sa pripadajucim stablom gradova, u listu
    }
    fclose(fp);
    return 0;
}
